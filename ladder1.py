...
class LadderNetwork:
    ...
    def build_decoder(self):
        with tf.name_scope('decoder'):
            ...
            for l in range(L, -1, -1):
                with tf.name_scope('layer_%d' % l):
                    print("Layer ", l, ": ", self.layers[l + 1] if l + 1 < len(self.layers) else None, " -> ", self.layers[l],
    ...
    def train_on_batch(self, x, y):
        self.session.run(self.train_step, feed_dict={self.inputs: x, self.outputs: y, self.training: 1})
            ...
            if i % 50 == 0:
                ...
                with open('train_log', 'a') as train_log:
                    ...
                    log_i = [i] + self.session.run([self.accuracy],
                                                 feed_dict={self.inputs: data.test.images, self.outputs: data.test.labels,
                                                            self.training: False})
